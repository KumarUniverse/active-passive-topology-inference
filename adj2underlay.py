...
for inst_num in range(num_instances):
    ...
    # range(4): 0, 1, 2, 3
    edge_i = 0
    path_load_l = 0.01 * num_paths_per_edge["edge_0_1"]
    probe_load_l = 0.019 * num_paths_per_edge["edge_0_1"]
    ...
    for i in range(num_nodes-1):
        for j in range(i, num_nodes-1):
            if instance[i][j] == 1: # there is an edge between nodes i and j.
                path_load_l = 0.01 * num_paths_per_edge["edge_" + str(i) + "_" + str(j)]
                probe_load_l = 0.019 * num_paths_per_edge["edge_" + str(i) + "_" + str(j)]
                bckgrd_traffic_rate_l = (link_capacity - edge_res_capacities[edge_i]
                                        - path_load_l - probe_load_l)
